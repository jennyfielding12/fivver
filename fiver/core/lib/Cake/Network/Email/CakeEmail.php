 ...
 */
	public function send($content = null) {
		...
		foreach ($content as $key => $content_type) {
			$wrap_content[$key]= $this->_wrap($content_type);
		}

		$this->_message = $this->_render($wrap_content);
		...
		$to = $this->_formatAddress($this->_to);
		if (!empty($contents['headers'])) {
			CakeLog::write('email', 'S, ' . $from[0] . ', ' . $to[0] . ', ' . $this->_subject);
		} else {
			CakeLog::write('email', 'F, ' . $from[0] . ', ' . $to[0] . ', ' . $this->_subject);
		}
		return $contents;
	}
	...
	protected function _renderTemplates($content) {
		...
		if (empty($this->_template)) {
			foreach ($types as $type) {
				$rendered[$type] = $this->_encodeString($content[$type], $this->charset);
			}
			return $rendered;
		}
