...
// From Siva, for sphinx -->
        if (isset($query['search'])) {
            ...
            if ($sphinxSearch) {
				...
				$containModel = array_keys($query['contain']);
                $associations = $this->getAssociated();
                if (!empty($associations)) {
                    foreach($associations as $model => $relationship) {
                        if (($relationship == 'belongsTo' || $relationship == 'hasOne') && in_array($model, $containModel)) {
 ...
 */
	function _clearPermanentCache() {
		if (PERMANENT_CACHE_CHECK === true) {
			$tmpUserIds = $tmpModelId = $tmpViewIds = array();
			$assoc[] = $this->alias;
			if (!empty($this->_permanentCacheAssociations)) {
				foreach ($this->_permanentCacheAssociations as $className) {
					$assoc[] = $className;
				}
			}
			if (Configure::read('permanent_cache.view_action')) {
				foreach(Configure::read('permanent_cache.view_action') as $tmpController) {
					$tmpModels[Inflector::classify($tmpController)] = Inflector::singularize($tmpController) . '_id';
				}
			}
			if (!empty($this->id)) {
				$tmpData = $this->find('first', array(
					'conditions' => array(
			...
			foreach($assoc as $tmpAssoc) {
				if (in_array($tmpAssoc, array_keys($tmpModels)) && (!empty($this->data[$tmpAssoc]['id']) || !empty($this->data[$this->alias][$tmpModels[$tmpAssoc]]))) {
					if (!empty($this->data[$tmpAssoc]['id'])) {
						$tmpModelId[$tmpAssoc] = $this->data[$tmpAssoc]['id'];
					}
					if (!empty($this->data[$this->alias][$tmpModels[$tmpAssoc]])) {
						$tmpModelId[$tmpAssoc] = $this->data[$this->alias][$tmpModels[$tmpAssoc]];
					}
				}
			}
			if (!empty($tmpData)) {
				foreach($tmpModels as $tmpModelKey => $tmpModelValue) {
					if (!empty($tmpData[$this->alias][$tmpModelValue])) {
						$tmpModelId[$tmpModelKey] = $tmpData[$this->alias][$tmpModelValue];
					}
				}
			}
			if (!empty($this->_permanentCacheAssociatedUsers) && !empty($tmpData)) {
				foreach($this->_permanentCacheAssociatedUsers as $v) {
					$tmpUserIds[$tmpData[$this->alias][$v]] = $tmpData[$this->alias][$v];
				}
			}
			if (!empty($this->_permanentCacheAssociatedViews) && !empty($tmpData)) {
				foreach($this->_permanentCacheAssociatedViews as $v) {
					$tmpViewIds[$v] = $tmpData[$this->alias][Inflector::singularize(Inflector::tableize($v)) . '_id'];
				}
			}
			if (empty($_SESSION['Auth']['User']['id']) && !empty($tmpData[$this->alias]['user_id'])) {
				$tmpUserIds[$tmpData[$this->alias]['user_id']] = $tmpData[$this->alias]['user_id'];
			}
			if (!empty($_SESSION['Auth']['User']['role_id']) && $_SESSION['Auth']['User']['role_id'] == ConstUserTypes::Admin && !empty($tmpData)) {
				if (!empty($tmpData[$this->alias]['user_id']) && $tmpData[$this->alias]['user_id'] != $_SESSION['Auth']['User']['id']) {
					$tmpUserIds[$tmpData[$this->alias]['user_id']] = $tmpData[$this->alias]['user_id'];
				}
			}
			clearPermanentCache($assoc, $tmpUserIds, $tmpModelId, $tmpViewIds);
			return true;
		}
	}
